\section{Proposed Reproducible Integrated Framework}
...
We use daily Brazilian equity prices from BovDB and BovDBv2 \cite{cardoso_2022_bovdb,souza_2024_bovdbv2}. BovDB provides an open historical B3 price database, while BovDBv2 extends the coverage and also contains intraday records. We retain daily adjusted closing prices because the objective is long-horizon cross-sectional analysis; daily sampling provides a common frequency across assets and limits the microstructure effects that are more prominent in intraday data. Adjustments for splits, dividends, interest on equity, bonuses and related corporate actions are essential because unadjusted jumps would contaminate returns, correlations and graph weights.
