...
No single metric provides a complete assessment, so the models are compared using complementary measures. MAE reports the average absolute prediction error. RMSE gives greater weight to large errors. Out-of-sample $R^2$ compares the model with a simple historical benchmark. QLIKE is a loss measure commonly used for volatility forecasts because it remains informative when realized volatility is an imperfect proxy for the underlying market risk \cite{patton_2011_volatility}. Lower MAE, RMSE and QLIKE values indicate better forecasts, while a higher out-of-sample $R^2$ indicates a stronger improvement over the benchmark.
